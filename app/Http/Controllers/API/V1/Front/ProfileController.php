...
class ProfileController extends BaseController
{
	...
	public function myCourses(Request $request)
    {
		...
		$userSubscription = UserSubscription::with('course')->withCount('videoWatchReport')->where("user_id",Auth::guard('api')->user()->id)->where("expired_on",">=",$currentDate)->where("status",'Success')->paginate(10);
	...
	public function myCourseDetails(Request $request)
    {
		...
		{	
			...
			$review = Rating::where(['course_video_id' => $data['course_id'],'status' => 1])->with(['user'])->paginate(5);
